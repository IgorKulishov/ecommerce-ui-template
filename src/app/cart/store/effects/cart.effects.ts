...
import {
  ...
  REMOVE_FROM_CART,
  SAVE_PLACED_ORDER,
  DELETE_ORDER_FROM_HISTORY_API,
  FETCH_ORDERS_HISTORY,
  FETCH_ALL_ORDERS_HISTORY,
  SAVE_ORDER_IN_HISTORY_API,
  AddToCart,
  AddToCartSuccess,
  StoreCurrentOrder,
  GetCurrentOrderFromStoreSuccess,
  CheckOutSuccess,
  SavePlacedOrder,
  SaveOrderInHistoryApi,
  StoreProcessedOrderInHistoryApiSuccess,
  FetchOrderHistory,
  StoreAllOrdersApiSuccess,
...
export class CartEffects {
  constructor(
    private addToCartAction$: Actions,
  ...
  @Effect() addToCart$: any = this.addToCartAction$.pipe(
    ofType(ADD_TO_CART),
    switchMap((productToCart: any) =>
      this.cartService.addProductToShoppingCart(productToCart).pipe(
  ...
  @Effect()
  saveProductsInStoreAPI$: any = this.saveProductsInStoreAPIAction$.pipe(
    ofType(ADD_TO_CART_SUCCESS),
    switchMap((productToCart: any) =>
      this.cartService.productsShoppingCart().pipe(
  ...
  @Effect() removeFromCart$: any = this.removeFromCartAction$.pipe(
    ...
    switchMap((removeProductInfo: any) =>
      this.cartService.removeProductFromShoppingCart(removeProductInfo).pipe(
        map((data: any) => new AddToCartSuccess(data)),
        catchError((err) =>
          of(new EffectError({ error_message: "remove_product_error" }))
        )
      )
    )
  );
  ...
  @Effect() checkoutShoppingCart$: any = this.checkoutShoppingCartAtion$.pipe(
    ofType(CHECK_OUT),
    map((data) => {
      this.sessionService.setPlacedOrderNumberInStorage(
        this.sessionService.getOrderNumberFromStorage()
      );
      return data;
    }),
    switchMap((orderPaymentDetails: any) =>
      this.cartService.checkoutShoppingCart(orderPaymentDetails.payload)
    ),
    switchMap((data) => [
  ...
  // get order from store
  @Effect()
  getSubmittedOrdersFromHistoryApi$: any = this.getOrdersFromtStoreAction$.pipe(
    ofType(FETCH_ORDERS_HISTORY),
    switchMap(() =>
      this.ordersHistoryService.fetchOrdersHistory().pipe(
        map((data: any) => {
          return new StoreProcessedOrderInHistoryApiSuccess(data.body);
        }),
        catchError((err) => {
          console.error(err);
  ...
  @Effect()
  getAllOrdersFromHistoryApi$: any = this.getAllOrdersFromtStoreAction$.pipe(
  ...
  @Effect()
  saveSubmittedOrderToHistoryApi$: any = this.saveOrderInHistoryApiAction$.pipe(
    ofType(SAVE_ORDER_IN_HISTORY_API),
    switchMap(() =>
      this.ordersHistoryService.saveOrder().pipe(
        map((data: any) => {
  ...
  @Effect()
  deleteSubmittedOrderFromApi: any = this.deleteOrderInHistoryApiAction$.pipe(
    ofType(DELETE_ORDER_FROM_HISTORY_API),
    switchMap((deleteOrderObj: { type: string; payload: any }) =>
      this.ordersHistoryService.deleteOrder(deleteOrderObj.payload).pipe(
        map((data: any) => {
