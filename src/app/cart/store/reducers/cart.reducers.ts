...
import {
  ADD_TO_CART,
  ADD_TO_CART_SUCCESS,
  SAVE_CURRENT_ORDER_IN_STORE,
  GET_CURRENT_ORDER_FROM_STORE,
  SAVE_PLACED_ORDER,
  GET_CURRENT_ORDER_FROM_STORE_SUCCESS,
  CHECK_OUT,
  CHECK_OUT_SUCCESS,
  REMOVE_FROM_CART,
...
const addProductToCartSuccess = (
  ...
  const newData: CartState = Object.assign({}, state, {
    productToCartSuccess: action.payload,
  });
  return newData;
};
...
const storeCurrentOrderInCart = (
  state: CartState,
  action: ReducerClass
): CartState => {
  const newData: CartState = Object.assign({}, state, {
    currentOrderInCart: action.payload,
  });
  return newData;
};
//TODO: finilize readCurrentOrderFromStore & readCurrentOrderFromStoreSuccess:
const readCurrentOrderFromStore = (
  state: CartState,
  action: ReducerClass
): CartState => {
  return state;
};
const readCurrentOrderFromStoreSuccess = (
  state: CartState,
  action: ReducerClass
): CartState => {
  return state;
...
const savePlacedOrderDetails = (
  state: CartState,
  action: ReducerClass
): CartState => {
  const newData: CartState = Object.assign({}, state, {
    processedOrdersDetails: action.payload,
  });
  return newData;
};
// There is no actual use of the (same) order details processed also saved in history lambda API
const storeProcessedOrderInHistoryApi = (
  state: CartState,
  action: ReducerClass
): CartState => {
  const newData: CartState = Object.assign({}, state, {
    orderStoredInHistoryApi: action.payload,
  });
  return newData;
};
const storeAllOrderApiSuccess = (
  state: CartState,
  action: ReducerClass
): CartState => {
  const newData: CartState = Object.assign({}, state, {
